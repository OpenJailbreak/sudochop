...
vm_address_t find_tlb(kernel_t* kernel) {
    int x = 0;
    void* data = malloc(0xB00000);
    if(data == 0) return 0;
    
    vm_address_t base = kernel->base;
    x = kernel->read(base, data, 0xB00000);
    if(x) {
        error("Unable to read mach-o header from kernel");
        return 0;
    }
    
    // TODO: Switch this to only require 4/8 byte reads
    macho_t* macho = macho_load(data, 0xB00000);
    if(macho) {
        //macho_debug(macho);
        macho_section_t* data_section = macho_get_section(macho, "__DATA", "__data");
        if(data_section) {
            //macho_section_debug(data_section);
            macho_section_t* common_section = macho_get_section(macho, "__DATA", "__common");
            if(common_section) {
                //macho_section_debug(common_section);
                void* data_buf = malloc(data_section->info->size);
                if(data_buf) {
                    x = kernel->read(data_section->info->addr, data_buf, data_section->info->size);
                    if(x) {
                        error("Unable to read in data section");
                        return 0;
                    }
                    
                    int i = 0;
                    for(i = 0; i < data_section->info->size; i+=4) {
                        unsigned char* ptr = &data_buf[i];
                        unsigned int value = *(unsigned int*)ptr;
                        // Check of this points into __DATA:__common section
                        if(value > common_section->info->addr
                                && value < common_section->info->addr + common_section->info->size) {
                            // I hope we can assume the first one found is kernel_pmap
                            if((kernel->get(value) != 0) && ((kernel->get(value) & 0xFFF) == 0) && (kernel->get(value) > value)) {
                            debug("Found TLB at 0x%08x with value 0x%08x", value, kernel->get(value));
                            macho_free(macho);
                            free(data_buf);
                            free(data);
                            return value;
                ...
                // Ok found our segments, let's see how many pointers there are
            }
        }
        macho_free(macho);
        
    } else {
        error("Unable to load kernel as mach-o file");
        free(data);
        return 0;
    }
    
    syslog(LOG_ERR, "Failed to find TLB offset");
    free(data);
    return 0;
}
...
uint32_t find_aes(uint32_t region, uint8_t* kdata, size_t ksize) {
    ...
    for(i = 0; i < ksize; i++) {
        ...
        if(memcmp(&kdata[i], "\xB0\xF5\xFA\x6F\x00\xF0", 6) == 0) {
            syslog(LOG_ERR, "Found IOAESAccelerator UID ptr at address 0x%08x", &kdata[i+4]);
            return i+4;
        }
    }
    syslog(LOG_ERR, "IOAESAccelerator Kernel patching failed\n");
    return 0;
}
...
vm_address_t find_aes(kernel_t* kernel) {
    ...
    vm_address_t addr = base + 0x1000;
    while(addr < (base + 0xB00000)) {
        memset(buf, '\0', 2048);
        x = kernel->read(addr, buf, 2048);
        if(x) {
            syslog(LOG_ERR, "Error reading in kernel");
            break;
        }
        ...
        for(i = 0; i < 2048; i++) {
            ...
            //"IOAESAccelerator enable UID" : (h("67 D0 40 F6"), h("00 20 40 F6")),
            //if (*((uint32_t*)&p[i]) == 0xF640d067) {
            if(memcmp(&p[i], "\xB0\xF5\xFA\x6F\x00\xF0", 6) == 0) {
                addr += i + 4;
                //syslog(LOG_ERR, "Found IOAESAccelerator UID ptr at 0x%08x, patching kernel", (uint32_t)(addr));
                free(buf);
                return addr;
            }
        }
